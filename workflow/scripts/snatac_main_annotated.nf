...
// Nature Biotechnology paper on Nextflow https://www.nature.com/articles/nbt.3820?proof=t%C2%A0

// DSL = "domain specific language", enables syntax extension that allows
// custom functions to be defined
// - processes can be defined but not automatically run by omitting the "from" and "into" channel declarations
// - defining a "workflow"
...
process prune {
    ...
    memory '3 GB'
    time '5h' //THIS SEEMS LIKE NOT ENOUGH TIME
    errorStrategy 'retry'
    maxRetries 2
    tag "${library} ${genome}"
    container 'library://porchard/default/general:20220107'
...
workflow {
    ...
    // the combine operator outputs cartesian product of items from two channels. when the "by" option is used
    // it combines only those times that share a common key, where the by parameter indicated the index of this key
    // for example:
