...
# parser.add_argument('--fastq-atac-dir', dest="atacdir", default = None, required = True, help = 'Location of multiome ATAC fastq files.')
# parser.add_argument('--fastq-gex-dir', dest="gexdir", default = None, required = True, help = 'Location of multiome GEX fastq files.')
...
for x in f:
    ...
    if args.modality == 'ATAC':
        ...
        for l in [ 'a', 'b', 'c', 'd']:
            ...
            index=args.dir + "/" + sample_id + "-ATAC_" + l + "_R2_001.fastq.gz"
            pe1=args.dir + "/" + sample_id + "-ATAC_" + l + "_R1_001.fastq.gz"
            pe2=args.dir + "/" + sample_id + "-ATAC_" + l + "_R3_001.fastq.gz"
            if os.path.isfile(index) and os.path.isfile(pe1) and os.path.isfile(pe2):
                libraries[sample_id]["readgroups"][group]["index"] = index
                libraries[sample_id]["readgroups"][group]["1"] = pe1
                libraries[sample_id]["readgroups"][group]["2"] = pe2
            else:
                print("ERROR: Missing expected ATAC fastq files.")
                exit()
    elif args.modality == "GEX":
        ...
        for l in [ 'a' ]:  ### Note this is just a dummy value to accommodate expected json format expected by nf pipeline
            group = sample_id + "_" + l
            libraries[sample_id]["readgroups"][group] = {}
            r1=args.dir + "/" + sample_id + "-3GEX_R1_001.fastq.gz"
            r2=args.dir + "/" + sample_id + "-3GEX_R2_001.fastq.gz"
