...
def get_clusters_for_set(set):
    d = pd.read_csv("results/liger/multimodal_"+set+"/cluster_to_cell_type.csv")
    return d['cell_type'].unique().tolist()


samples = get_samples_for_set("all")
clusters = get_clusters_for_set("all")
...
rule all:
    input:
        expand(_results("cellranger_barcodes_sample_cluster/barcodes_{sample}_{cluster}.txt"), sample=samples, cluster=clusters),
        expand(_results("scafe_by_cluster/{cluster}/ctss_to_bigwig/{cluster}/wig/{cluster}.cpm.fwd.bw"), cluster=clusters),
        expand(_results("scafe_by_cluster/{cluster}/annotate/{cluster}/bed/{cluster}.CRE.annot.bed"), cluster=clusters),


rule bgzip:
    input:
        barcodes ="results/fiveprime/droplet_utils/{sample}/barcodes_nuclei.txt",
    output:
        barcodes_gz = "results/fiveprime/droplet_utils/{sample}/barcodes_nuclei.txt",
    conda:
        "genetics"
    shell:
        """
...
rule scafe_by_sample:
    input:
        ...
        barcodes ="results/fiveprime/droplet_utils/{sample}/barcodes_nuclei.txt.gz",
    output:
...
rule cellranger_barcodes_by_sample_by_cluster:
    ...
    output:
        barcodes = _results("cellranger_barcodes_sample_cluster/barcodes_{sample}_{cluster}.txt"),
    params:
        sample = "{sample}",
        cluster = "{cluster}",
    shell:
...
rule cellranger_bgzip:
    input:
        barcodes = _results("cellranger_barcodes_sample_cluster/barcodes_{sample}_{cluster}.txt"),
    output:
        barcodes_gz = _results("cellranger_barcodes_sample_cluster/barcodes_{sample}_{cluster}.txt.gz"),
    conda:
        "genetics"
    shell:
        """
...
rule cellranger_bam_by_sample_by_cluster:
    input:
        ...
        barcodes = _results("cellranger_barcodes_sample_cluster/barcodes_{sample}_{cluster}.txt"),
    output:
        bam = _results("cellranger_bam_sample_cluster/{sample}_{cluster}.bam"),
        bai = _results("cellranger_bam_sample_cluster/{sample}_{cluster}.bam.bai"),
    shell:
        """
...
# rule merge_bams:
#     input:
#         bams = lambda wildcards: expand(_results("cellranger_bam_sample_cluster/{{set}}/cellranger_{sample}_{{cluster}}.bam"), sample=get_samples_for_set(wildcards.set)),
#     output:
...
rule scafe_by_sample_by_cluster:
    input:
        bam = _results("cellranger_bam_sample_cluster/{sample}_{cluster}.bam"),
    output:
        bed = _results("scafe_by_sample_by_cluster/{sample}_{cluster}/ctss_to_bigwig/{sample}_{cluster}/bed/{sample}_{cluster}.unencoded_G.collapse.ctss.bed.gz"),
    params:
        tag = "{sample}_{cluster}",
        outdir = _results("scafe_by_sample_by_cluster/{sample}_{cluster}")
    shell:
        """
        scafe.workflow.sc.solo \
          --run_bam_path={input.bam} \
...
rule scafe_aggregate:
    input:
        beds = lambda wildcards:
            expand(_results("scafe_sample_cluster/{sample}_{{cluster}}/ctss_to_bigwig/{sample}_{{cluster}}/bed/{sample}_{{cluster}}.unencoded_G.collapse.ctss.bed.gz"),
            sample=get_samples_for_set(wildcards.set)),
        lib_list_path = _results("scafe_by_cluster/{set}/{cluster}_lib_list_path.txt"),
    output:
        cpm_fw = _results("scafe_cluster/{set}/{cluster}/ctss_to_bigwig/{cluster}/wig/{cluster}.cpm.fwd.bw"),
        cpm_rev = _results("scafe_cluster/{set}/{cluster}/ctss_to_bigwig/{cluster}/wig/{cluster}.cpm.rev.bw"),
    params:
        tag = "{cluster}",
        outdir = _results("scafe_cluster/{set}/{cluster}"),
    shell:
        """
...
rule unzip:
    input:
        bed = _results("scafe_cluster/{set}/{cluster}/annotate/{cluster}/bed/{cluster}.CRE.annot.bed.gz"),
    output:
        bed = _results("scafe_cluster/{set}/{cluster}/annotate/{cluster}/bed/{cluster}.CRE.annot.bed"),
    conda:
        "genetics"
    shell:
        """
